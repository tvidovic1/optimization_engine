...
// A function that finds the LCM of the n inputs in the vector relying on GCDsž
long long int findLeastCommonMultiple(std::vector<long long int> inputs);

// A function that returns the left most digit of a 3 digit number (0 if non-enxistent)
int getFirstDigit(int number);

// A function that returns the second left most digit of a 3 digit number (0 if non-enxistent)
int getSecondDigit(int number);

// A function that returns the right most digit of a 3 digit number
...
// A function that returns vector of n uniformly random number whose sum is equal to u (randFixedSum)
